...
import { ChevronUp, ChevronDown, Grid3x3, SkipForward, Volume2, VolumeX, Tv } from "lucide-react";
...
function Index() {
  ...
  }, [changeChannel, advance]);

  const upNext = useMemo(() => {
    const queue = queuesRef.current[channel.id];
    if (!queue) return null;
    return queue.order[(queue.cursor + 1) % queue.order.length];
  }, [channelIdx, q.cursor]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <section className="relative flex flex-1 flex-col lg:flex-row">
        ...
        <aside className="hidden w-64 shrink-0 border-r border-border/60 bg-black/30 lg:block">
          ...
          <div className="h-[calc(100vh-180px)] overflow-y-auto py-2">
            {CHANNELS.map((ch, i) => {
              const active = i === channelIdx;
              return (
                <button
                  key={ch.id}
                  onClick={() => { setChannelIdx(i); setTitle(""); }}
                  className={
        ...
        <div className="relative flex flex-1 flex-col">
          ...
          <div className="border-t border-border/60 bg-black/60 px-6 py-4">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1 font-mono-tv text-[10px] uppercase tracking-widest text-muted-foreground">
              ...
              <span>G guide</span>
              <span>M mute</span>
              {upNext && <span className="text-foreground/60">up next · {upNext}</span>}
            </div>
          </div>
        </div>
