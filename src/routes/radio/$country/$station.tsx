...
export const Route = createFileRoute("/radio/$country/$station")({
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        },
        {
          name: "description",
          content: country
            ? `Listen to a radio station from ${country.name} inside TubeTV.`
            : "The requested radio country could not be found.",
        },
      ],
    };
  },
  component: RadioStreamRoute,
});

function RadioStreamRoute() {
  const { country, station } = Route.useParams();
  ...
  return (
    <TubeTVPage
      initialMode="radio"
      initialRadioCountry={countryData.code}
      initialRadioItemSlug={station}
    />
  );
}
