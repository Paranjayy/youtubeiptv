...
export function TubeTVPage({
  ...
  const {
    ...
    updateSetting: updateAutoPilotSetting,
  } = useAutoPilot(autoPilotCallbacks);
  ...
  const [movieCast, setMovieCast] = useState<
    { name: string; character: string; profileUrl: string | null }[]
  >([]);
  ...
  const [movieSeasonsList, setMovieSeasonsList] = useState<
    { seasonNumber: number; episodeCount: number }[]
  >([]);
  const [movieDetailLoading, setMovieDetailLoading] = useState(false);
  const [activeDownloadStats, setActiveDownloadStats] = useState<
    Record<string, { downloaded: number; total: number; speed: number }>
  >({});

  // Helper to select movie or show and navigate to slug URL
  const selectMovieOrShow = useCallback(
    (item: MediaItem) => {
    ...
    fetch("https://api.counterapi.dev/v1/tubetv/visits/up")
      ...
      .then((data) => {
        if (data && typeof data.value === "number") {
          ...
          const activeBase = Math.max(
            12,
            Math.floor(Math.sqrt(data.value) * 0.6) + Math.floor(Math.random() * 5),
          );
          ...
          if (data.recommendations?.results) {
            const recList = data.recommendations.results.slice(0, 10).map((item: any) => ({
              ...
              genres: item.genre_ids
                ? item.genre_ids.map((gid: number) => getGenreName(gid))
                : ["Stream"],
  ...
  useEffect(() => {
    ...
    if (
      initialMovieId &&
      initialMovieType &&
      (selectedMedia.id !== initialMovieId || selectedMedia.type !== initialMovieType)
    ) {
      const matched = TRENDING_MEDIA.find(
        (m) => m.id === initialMovieId && m.type === initialMovieType,
      );
        ...
        <section className="relative flex flex-1 flex-col lg:flex-row min-h-0 pb-16 lg:pb-0">
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border/60 px-4 py-2.5">
              ...
              <span className="rounded bg-primary/15 px-1.5 py-0.5 font-mono-tv text-[9px] uppercase tracking-widest text-primary">
                {CHANNELS.length} ch
              </span>
            </div>
            ...
            <div className="border-b border-border/60 py-1.5">
              <button
                ...
                style={
                  mode === "iptv" ? { boxShadow: "inset 3px 0 0 oklch(0.84 0.14 205)" } : undefined
                }
                className={cn(
              ...
              >
                ...
                {mode === "iptv" && (
                  <span className="h-1.5 w-1.5 shrink-0 animate-pulse-dot rounded-full bg-[oklch(0.84_0.14_205)]" />
                )}
              </button>
              <button
                onClick={() => {
                ...
                style={
                  mode === "radio" ? { boxShadow: "inset 3px 0 0 oklch(0.86 0.16 72)" } : undefined
                }
                className={cn(
                  "flex w-full items-center gap-3 px-4 py-2.5 text-left transition-all duration-200",
                  mode === "radio"
              ...
              >
                ...
                {mode === "radio" && (
                  <span className="h-1.5 w-1.5 shrink-0 animate-pulse-dot rounded-full bg-[oklch(0.86_0.16_72)]" />
                )}
              </button>
              <button
                ...
                style={
                  mode === "movies"
                    ? { boxShadow: "inset 3px 0 0 oklch(0.74 0.18 335)" }
                    : undefined
                }
                className={cn(
              ...
              >
                ...
                <span className="flex-1">
                  <span
                    className="block text-[11px] font-semibold uppercase tracking-wider text-[oklch(0.74_0.18_335)]"
                    style={{ textShadow: "0 0 6px oklch(0.74 0.18 335 / 0.4)" }}
                  >
                    Movies & Series
                  </span>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              >
                <span className="font-mono-tv text-[9px] uppercase tracking-widest">
                  📺 YT Channels
                </span>
                {youtubeListCollapsed ? (
                  ...
                  return (
                    <div key={cat}>
                      ...
                      <div
                        ...
                        className={cn(
                          "flex items-center gap-1.5 border-b border-t border-border/40 px-3 py-1.5 cursor-pointer hover:brightness-110 select-none",
                          catColor.bg,
                        )}
                      >
                        <span
                          className={cn(
                            "font-mono-tv text-[9px] font-bold uppercase tracking-widest",
                            catColor.text,
                          )}
                        >
                          {catColor.label}
                        ...
                        <span
                          className={cn(
                            "ml-auto font-mono-tv text-[9px] opacity-60 mr-2",
                            catColor.text,
                          )}
                        >
                          {catChannels.length}
                          ...
                          return (
                            <button
                              ...
                              style={
                                active
                                  ? {
                                      boxShadow: `inset 3px 0 0 ${ch.color}`,
                            ...
                            >
                              <span
                                ...
                                style={
                                  active
                                    ? { color: ch.color, textShadow: `0 0 8px ${ch.color}` }
                                    : undefined
                                }
                              >
                                {ch.number}
              ...
              <div className="border-t border-border/60 p-2 grid grid-cols-4 gap-1 mt-4 bg-black/10">
                {(
                  ...
                  >
                    ...
                    <span
                      className="font-mono-tv text-[8px] uppercase tracking-wider"
                ...
                <div className="px-2 pt-2">
                  <AutoPilotPanel
                    ...
                    onSettingChange={(key, val) =>
                      updateAutoPilotSetting(key as "autoSwitchChannel", val as boolean)
                    }
              ...
              <div className="border-t border-border/60 bg-black/40 p-2.5 flex flex-col gap-2">
                ...
                >
                  ...
                  <span className="font-mono-tv text-[9px] uppercase tracking-[0.15em]">
                    GitHub Source
                  </span>
                </a>
              </div>
                ...
                <div className="relative w-full px-4 py-6 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col gap-8 min-h-full z-10">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="text-xs font-bold text-yellow-500 uppercase tracking-wider">
                            Bring Your Own API Key (TMDb)
                          </p>
                          <p className="text-[11px] text-zinc-400">
                            Offline Fallback Mode active. Enter your TMDb v3 API Key in the settings
                            card at the bottom to search the entire online database.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <section className="space-y-6 animate-fade-in min-h-[50vh]">
                      <div className="flex items-center justify-between border-b border-white/5 pb-3">
                        <h2 className="text-lg font-bold text-white">
                          Search Results for{" "}
                          <span className="text-red-500 italic">"{movieSearchQuery}"</span>
                        </h2>
                      ...
                      {movieSearchLoading ? (
                        ...
                        <div className="text-center py-20 text-zinc-500 border border-dashed border-white/5 rounded-3xl">
                          ...
                          <p className="text-[11px] text-zinc-600 mt-1">
                            Try another keyword, or supply a TMDb key below for online lookup.
                          </p>
                        </div>
                      )}
                    </section>
                      ...
                      <div className="grid gap-6 lg:grid-cols-[1.6fr_0.8fr] text-left">
                        ...
                        <div className="space-y-4">
                          <div className="relative aspect-video w-full overflow-hidden rounded-[2.5rem] border border-white/10 bg-zinc-950/60 shadow-2xl shadow-black/85 group">
                            ...
                            />
                            <iframe
                              src={VIDEO_SOURCES[movieActiveSourceIndex].getUrl(
                                selectedMedia.id,
                                movieImdbId,
                                selectedMedia.type,
                                movieSeason,
                          ...
                          <div className="rounded-[2rem] border border-white/10 bg-zinc-950/40 p-5 backdrop-blur-md">
                            <div className="flex items-center justify-between flex-wrap gap-2">
                              ...
                              <div className="flex items-center gap-2 text-xs text-zinc-500">
                                <AlertTriangle className="h-3.5 w-3.5 text-yellow-500" /> Buffering?
                                  ...
                                  <>
                                    <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                                      ...
                                      <div className="flex items-center gap-1 bg-black/45 border border-white/5 rounded-full p-0.5 flex-wrap max-w-full overflow-x-auto">
                                        ...
                                        {(movieSeasonsList.length > 0
                                          ? movieSeasonsList
                                          : Array.from({ length: movieTotalSeasons }).map(
                                              (_, i) => ({
                                                seasonNumber: i + 1,
                                                episodeCount: movieEpisodesCount,
                                              }),
                                            )
                                        ).map((season) => (
                                          <button
                                            ...
                                            className={cn(
                                              ...
                                              movieSeason === season.seasonNumber
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="rounded-2xl bg-black/45 border border-white/[0.05] divide-y divide-white/[0.05] font-mono text-[9px] w-full">
                                  <div className="flex items-center justify-between px-3.5 py-2">
                                    ...
                                    <span className="text-zinc-200 font-bold truncate max-w-[150px]">
                                      {movieExtraDetails.director}
                                    </span>
                                  </div>
                                  <div className="flex items-center justify-between px-3.5 py-2">
                                    <span className="text-zinc-500">LANGUAGE</span>
                                    <span className="text-zinc-200 font-bold uppercase">
                                      {movieExtraDetails.language}
                                    </span>
                                  </div>
                                  <div className="flex items-center justify-between px-3.5 py-2">
                                    <span className="text-zinc-500">RELEASED</span>
                                    <span className="text-zinc-200 font-bold">
                                      {movieExtraDetails.releaseDate !== "N/A"
                                        ? movieExtraDetails.releaseDate
                                        : selectedMedia.year}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="flex gap-2">
                                  {selectedMedia.trailers && selectedMedia.trailers.length > 0 && (
                                    <button
                                      onClick={() =>
                                        setMovieActiveTrailerKey(
                                          selectedMedia.trailers?.[0].key ?? null,
                                        )
                                      }
                          ...
                          <section className="space-y-3 text-left">
                            <div className="flex items-center justify-between px-1">
                              ...
                              <span className="text-[10px] font-mono text-zinc-500">
                                {movieRecommendations.length} titles
                              </span>
                            </div>
                            <div className="flex gap-4 overflow-x-auto pb-4 pt-1 scrollbar-hide items-start">
                  ...
                  <section className="grid gap-6 md:grid-cols-2 text-left mt-2">
                    ...
                    <article className="rounded-[2rem] border border-white/10 bg-zinc-950/40 p-5 backdrop-blur-md">
                      <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-widest text-zinc-300 font-bold">
                        ...
                        {tmdbKey.trim() ? (
                          <span className="text-emerald-400 font-bold text-[9px] border border-emerald-500/20 bg-emerald-500/5 px-2 py-0.5 rounded-full">
                            Connected
                          </span>
                        ) : (
                          <span className="text-yellow-500 font-bold text-[9px] border border-yellow-500/20 bg-yellow-500/5 px-2 py-0.5 rounded-full">
                            Offline Mode
                          </span>
                        )}
                      </div>
                      <div className="mt-3.5 space-y-3">
                        <p className="text-[10px] text-zinc-400 leading-relaxed font-mono">
                          Paste your personal TMDb v3 API Key to resolve online details, search,
                          cast & trailers. Saved strictly to localStorage.
                        </p>
                  ...
                  {movieCast.length > 0 && (
                    <section className="space-y-4 mt-2 text-left">
                      <h2 className="text-xl font-bold text-white/90 px-2 uppercase tracking-tight">
                        Cast Members
                      </h2>
                      <div className="flex gap-5 overflow-x-auto p-4 px-6 scrollbar-hide">
                        {movieCast.map((member) => (
                          <div
                            key={member.name}
                            className="flex flex-col items-center gap-2.5 flex-none w-28 text-center group"
                          >
                    ...
                    <section className="rounded-[2rem] border border-white/10 bg-[#0c0f16]/90 p-6 backdrop-blur-xl animate-fade-in text-left">
                      <div className="flex items-center justify-between border-b border-white/5 pb-3">
                        <div className="flex items-center gap-2">
                          <svg
                            className="w-5 h-5 text-red-500 fill-current animate-bounce"
                            viewBox="0 0 24 24"
                          >
                            <path d="M5 20h14v-2H5v2zM19 9h-4V3H9v6H5l7 7 7-7z" />
                          </svg>
                          <span className="font-mono text-xs uppercase tracking-widest text-zinc-100 font-bold">
                            Offline Download Cabinet
                          </span>
                        </div>
                        <button
                          onClick={() => setMovieShowDownloadPanel(false)}
                        ...
                        {(() => {
                          ...
                          return sizeTable.map(({ quality, size, format }) => {
                              ...
                              const interval = setInterval(() => {
                                setActiveDownloadStats((prev) => {
                                  ...
                                  const nextDownloaded = Math.min(
                                    curr.total,
                                    curr.downloaded + chunk,
                                  );
                                  const percentage = Math.round(
                                    (nextDownloaded / curr.total) * 100,
                                  );

                                  setMovieDownloadProgress((prog) => ({
                                    ...prog,
                                    [key]: percentage,
                                  }));
                            ...
                            return (
                              <div
                                key={quality}
                                className="rounded-xl border border-white/5 bg-white/[0.02] p-4 flex flex-col justify-between gap-3 hover:border-white/10 hover:bg-white/[0.04] transition-all"
                              >
                                <div>
                                  ...
                                  <div className="text-xs text-zinc-500 font-mono mt-1">
                                    {format} ·{" "}
                                    {isActive && stats
                                      ? `${formatBytes(stats.downloaded)} of ${formatBytes(stats.total)}`
                                      : size}
                                  </div>
                                  {isActive && stats && (
                  ...
                  <div className="mt-8 p-4 font-mono text-[9px] leading-relaxed text-zinc-600 text-center uppercase tracking-wider border-t border-white/5">
                    Disclaimer: This app indexes third party streaming APIs for educational
                    research. No files are stored locally.
                  </div>
                </div>
              </div>
            ) : (
